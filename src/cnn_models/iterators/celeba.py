...
class CelebADataset(object):
    ...
    def __init__(self, data_path="./train", resolution="32x32", normalize_max=True, train_set=True):
        self.position = 0
        ...
        if train_set:
            self.celeba_images = self.dataset_imgs[self.dividing_point:]
        else:
            self.celeba_images = self.dataset_imgs[:self.dividing_point]
        random.shuffle(self.celeba_images)
        self.res_tuple = [int(x_s) for x_s in resolution.split('x')]

    def next_batch(self, number):
        first_file_name = self.celeba_images[self.position]
        ...
        self.position += 1
        self.position = self.position if self.position < len(self.celeba_images) else 0

        # iterate over elements
        for img_idx in range(1, number):
            img_name = self.celeba_images[self.position]
